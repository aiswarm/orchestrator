 ...
 * | `messageUpdated`             | `(message)` the {@link Message}                      | {@link Message} status setter      | A message's status or metadata changed after dispatch. Not fired for content appends — see `messageAppended`. |
 * | `messageAppended`            | `(message, delta)` the {@link Message} and the new chunk | {@link Message#append}         | A streaming driver appended a chunk to the message content. Subscribers can apply the delta locally instead of re-shipping the full snapshot. |
